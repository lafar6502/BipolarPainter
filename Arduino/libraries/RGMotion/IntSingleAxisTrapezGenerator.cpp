...
void IntSingleAxisTrapezGenerator::NextStep() {
	...
	if (_stage == 2 || _stage == 1) {
		if (_t >= _brakeT) {
			_stage = 3;
			double remd = (_targetPos - _curPos) * 2.0;
			double remt = _motionT - _t;
			//18t, 3250 dist
			//remd = _decel * remt * remt / 2 => decel = remd * 2 / remt * remt
			_decel = remd / (remt * remt);
...
double IntSingleAxisTrapezGenerator::GetDeceleration() {
	return _decel;
}
